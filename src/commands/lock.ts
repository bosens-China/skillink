...
export async function lockCommand(options: { cwd?: string; files?: string[] }) {
  ...
  if (files.length === 0) {
    console.log(pc.yellow(t('没有需要加密的文件', 'No files to encrypt', locale, config.locale)));
    return;
  }
  ...
  for (const file of files) {
    ...
    if (!existsSync(filePath)) {
      console.warn(pc.yellow(t(`文件不存在，跳过: `, 'File not found, skipping: ', locale, config.locale) + file));
      continue;
    }
  ...
  if (existingFiles.length === 0) {
    console.log(pc.yellow(t('没有可加密的文件', 'No files available to encrypt', locale, config.locale)));
    return;
  }

  // 提示输入密码
  const pwd = await password({
    message: t('输入加密密码', 'Enter encryption password', locale, config.locale),
    mask: '*',
  });
  ...
  console.log(
    pc.green(
      t(`✓ ${count} 个文件已加密`, `✓ ${count} file(s) encrypted`, locale, config.locale),
    ),
  );
}
