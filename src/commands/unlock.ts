...
export async function unlockCommand(options: {
  ...
  if (files.length === 0) {
    console.log(
      pc.yellow(
        t('没有需要还原的文件', 'No files to unlock', locale, config.locale),
      ),
    );
    return;
  }
  ...
  if (existingLocks.length === 0) {
    console.log(
      pc.yellow(
        t(
          '没有可还原的锁文件',
          'No lock files available to unlock',
          locale,
          config.locale,
        ),
      ),
  ...
  for (const file of existingLocks) {
    ...
    console.log(
      pc.green(`+ ${path.basename(file)}.lock -> ${file}`) +
        pc.gray(` ${t('(已还原)', '(decrypted)', locale, config.locale)}`),
    );
    count++;
  }
  ...
  if (failed > 0) {
    console.log(
      pc.yellow(
        t(
          `⚠ ${failed} 个文件解密失败（密码错误或文件损坏）`,
          `⚠ ${failed} file(s) failed to decrypt (wrong password or corrupted)`,
          locale,
          config.locale,
        ),
      ),
