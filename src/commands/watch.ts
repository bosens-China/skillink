...
export async function watch(cwd: string = process.cwd()): Promise<void> {
  ...
  watcher
    .on('add', (filePath) => {
      console.log(pc.gray(`[${new Date().toLocaleTimeString()}] 添加: ${path.relative(cwd, filePath)}`));
      debouncedSync();
    })
    .on('change', (filePath) => {
      console.log(pc.gray(`[${new Date().toLocaleTimeString()}] 修改: ${path.relative(cwd, filePath)}`));
      debouncedSync();
    })
    .on('unlink', (filePath) => {
      console.log(pc.gray(`[${new Date().toLocaleTimeString()}] 删除: ${path.relative(cwd, filePath)}`));
      debouncedSync();
    })
